...
public class WebRtcChannelLoopbackTest
{
    ...
    /// </summary>
    [Theory(Timeout = 180000, Skip = "Diagnostic benchmark — intentionally 'fails' to report timing. Remove Skip to run.")]
    [InlineData(1)]
    [InlineData(2)]
    [InlineData(4)]
    [InlineData(8)]
    public async Task Benchmark_MultiChannelThroughput_18MB(int laneCount)
    {
        var paired = new PairedFakeSignaling();
        var config = new RTCConfiguration();
        ...
        await using var offerer = new MultiWebRtcChannel(
            config, paired.Offerer, WebRtcRole.Offerer, laneCount: laneCount);
        await using var answerer = new MultiWebRtcChannel(
            config, paired.Answerer, WebRtcRole.Answerer, laneCount: laneCount);
        ...
        const int targetBytes = 18_500_000;
        const int chunkSize = 262_135;
        var chunkCount = targetBytes / chunkSize;
        ...
        var receiveTask = Task.Run(async () =>
        {
            ...
            {
                total += (await answerer.ReceiveAsync(cts.Token)).Length;
            }
        ...
        var totalBytes = await receiveTask;
        sw.Stop();

        var mbPerSec = totalBytes / 1024.0 / 1024.0 / sw.Elapsed.TotalSeconds;
        throw new Xunit.Sdk.XunitException(
            $"BENCHMARK RESULT (not a failure): lanes={laneCount}, " +
            $"connect={connectSw.Elapsed.TotalSeconds:F2}s, {chunkCount} chunks, " +
            $"{totalBytes:N0} bytes, {sw.Elapsed.TotalSeconds:F2}s, {mbPerSec:F2} MB/s");
    }
}
